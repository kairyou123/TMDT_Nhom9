...
class ProductEdit extends React.Component {
    ...
    getData = () => {
        Promise.all([axios.get('https://localhost:5001/api/product/' + this.state.productSlug), 
            ...
            this.setState({
                ...
                srcImage: "https://localhost:5001/Image/" + result[0].data.image,
            ...
            this.setState({
                ...
                catalog: this.state.product.catalog.id,
                manufactuer: this.state.product.producer.id,
            });
            if(this.state.product.subImage) {
                this.setState({
                    srcSubImage: "https://localhost:5001/Image/" + this.state.product.subImage,
                })
            }
        });
    ...
    previewImage = () => {
        ...
        if(file) {
            ...
            this.setState({
                srcImage: "https://localhost:5001/Image/" + this.state.product.image,
            })
        }
    ...
    handleImageError = () => {
        return new Promise((resolve, reject) => {
            if(!this.state.image) {
                reject("Không có hình ảnh nào để upload");
            }
            resolve("ok");
        })
    }
    ...
    handleSubmit = async (e) => {
        ...
        var image = (this.state.image !== "" && typeof this.state.image === 'string') ? this.state.image : this.state.product.image;
        var subImage = (this.state.subImage !== "" && typeof this.state.subImage === 'string') ? this.state.subImage : this.state.product.subImage;
        var data = {
            'Id': parseInt(this.state.product.id),
        ...
        axios.put('https://localhost:5001/api/product/' + this.state.product.id, data, {
                headers: {
                    'Content-Type': 'application/json',
                }
        })
        .then(result => {
            var url = "/admin/product/" + result.data + '/edit';
    ...
    render() {
        const {catalogs, manufactuers, srcSubImage} = this.state;
       ...
       var manufactuers_select = manufactuers.map((manufactuer, i) => {
        ...
        return (
            <div>
                <div className="admin-content-title"><b>SỬA ĐỔI SẢN PHẨM</b></div>
                    ...
                    <form id="productEdit" name="productEdit" onSubmit={this.handleSubmit}>
                        ...
                        <div className="form-group row">
                            ...
                            <div className="col-sm-8">
                                <img src={this.state.srcImage} width="100px" height="100px"></img>
                            </div>
                        </div>
                        <div className="form-group row">
                        ...
                        <div className="form-group row">
                            ...
                            <div className="col-sm-8">
                                <input type="number" className="form-control" id="discount" placeholder="Nhập phần trăm giảm giá"  value={this.state.discount || 0} onChange={this.handleChange} name="discount"  />
                            </div>
                        </div>
                        <div className="form-group row">
