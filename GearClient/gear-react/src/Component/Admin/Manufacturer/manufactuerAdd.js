...
class CatalogAdd extends React.Component {
    ...
    handleSubmit = async  (e) => {
        ...
        axios.post("https://localhost:5001/api/producer", data, {
            headers: {
                'Content-Type': 'application/json',
                    }
             })
             .then(result => {
                 var success_ = "Tạo thành công hãng sản xuất " + result.data.name;
                this.setState({
                    success: success_,
                });
        });
    ...
    render() {
        return (
            <div>
                <div className="admin-content-title"><b>THÊM HÃNG SẢN XUẤT</b></div>
                <div className="admin-content">
                    ...
                    {this.showError()}
                    <form id="manufactuerAdd" name="manufactuerAdd" onSubmit={this.handleSubmit}>
                        ...
                        <div className="form-group row">
                            <div className="admin-form-edit-button">
                                <button className="btn btn-warning admin-form-edit-button" type="submit" form="manufactuerAdd" value="Submit" onClick={this.handleSubmit}>Tạo</button>
                            </div>
                        </div>
                    </form>
                    </div>
